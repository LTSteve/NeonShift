...
public class PlayerController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // X axis movement
        if (Mathf.Abs(Input.GetAxisRaw(PC2D.Input.HORIZONTAL)) > PC2D.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = Input.GetAxisRaw(PC2D.Input.HORIZONTAL);
        }
        else
        ...
        if (Input.GetAxisRaw(PC2D.Input.VERTICAL) != 0)
        {
            bool up_pressed = Input.GetAxisRaw(PC2D.Input.VERTICAL) > 0;
        ...
        else if (Input.GetAxisRaw(PC2D.Input.VERTICAL) < -PC2D.Globals.FAST_FALL_THRESHOLD)
        {
